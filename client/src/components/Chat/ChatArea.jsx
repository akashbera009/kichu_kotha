...
import Message from "./Message";
import MessageInput from "./MessageInput";
import VideoCallModal from "./VideoCallModal"; // New component we'll create
import useVideoCall from "../../hooks/useVideoCall"; // The hook we created
import "./ChatArea.css";
...
const ChatArea = ({ selectedUser, socket }) => {
  ...
  const {
    ...
    toggleVideo,
    toggleAudio
  } = useVideoCall(socket); // Pass the existing socket

  // Track if we should scroll to bottom
  const shouldScrollToBottomRef = useRef(true);
  const isInitialLoadRef = useRef(true);

  // Register user for video calls when component mounts
  useEffect(() => {
  ...
  }, [currentUser, socket, registerForCalls]);

  // Handle incoming calls - show modal automatically
  useEffect(() => {
  ...
  // Handle call ended - close modal
  useEffect(() => {
    if (callEnded) {
      setShowVideoCall(false);
      // Reset call ended state after a brief delay
      setTimeout(() => {
        // You might want to add a resetCall function to your hook
      }, 1000);
    }
  }, [callEnded]);
  ...
  const handleVideoCallClick = () => {
    ...
    // Find the selected user's socket ID from the users list
    const targetUser = users.find(user => user.userId === selectedUser._id);
    if (targetUser) {
      setShowVideoCall(true);
      callUser(targetUser.id); // Use socket ID for calling
    } else {
      alert("User is not available for video call");
    }
  };

  // Handle voice call (placeholder for future implementation)
